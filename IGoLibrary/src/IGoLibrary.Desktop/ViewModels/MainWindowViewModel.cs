...
using IGoLibrary.Desktop.Platform;
    ...
    ITaskLaunchHistoryService? taskLaunchHistoryService = null) : ViewModelBase
{
    ...
    public string[] GrabTaskTargets { get; } = ["今日抢座", "明日预约"];
    ...
    public bool HasReservationRecords => _homeReservationRecords.Count > 0;

    public bool HasNoReservationRecords => !HasReservationRecords;
    ...
    public IReadOnlyList<ReservationRecordViewModel> HomeReservationRecords => _homeReservationRecords;
    ...
    [ObservableProperty]
    private bool isRefreshingReservationRecords;
    ...
    [ObservableProperty]
    private int selectedGrabTaskTargetIndex;
    ...
    [ObservableProperty]
    private string updateStatusText = $"当前版本 {appUpdateService.CurrentVersionText}";
    ...
    public bool IsTomorrowReservationTarget => SelectedGrabTaskTargetIndex == 1;
    ...
    public string SelectedSeatHintText => HasSelectedSeats
        ? IsTomorrowReservationTarget
            ? "这些座位会用于明日预约。任务会先进入排队通道，再循环提交明日预约请求。"
    ...
    {
        RefreshReservationCommand.NotifyCanExecuteChanged();
    }
    ...
    private bool CanRefreshReservationRecords()
    {
        return !IsRefreshingReservationRecords;
    }

    partial void OnReReserveDelaySecondsChanged(int value)
    {
        UpdateReservationCountdown();
    }

    partial void OnReReserveLeadMinutesChanged(int value)
    {
        UpdateReservationCountdown();
    }
    ...
    partial void OnIsGrabTaskActiveChanged(bool value)
    {
        ...
        OnPropertyChanged(nameof(CanEditGrabReservationStrategy));
        OnPropertyChanged(nameof(CanStartRandomAvailableSeatGrab));
    }

    partial void OnSelectedGrabTaskTargetIndexChanged(int value)
    {
        OnPropertyChanged(nameof(IsTodayGrabTarget));
        OnPropertyChanged(nameof(IsTomorrowReservationTarget));
    ...
    public async Task InitializeAsync()
    {
        ...
        tomorrowReservationCoordinator.StatusChanged += OnTomorrowReservationStatusChanged;
        occupySeatCoordinator.StatusChanged += OnOccupyStatusChanged;
        ApplyGrabStatus(grabSeatCoordinator.GetStatus());
        if (IsTaskActive(tomorrowReservationCoordinator.GetStatus()))
        {
            ApplyGrabStatus(tomorrowReservationCoordinator.GetStatus());
        }
    ...
    {
        OpenUrl("https://github.com/Luofaiz/IGoLibrary/releases");
    }
    ...
    private async Task CheckForUpdatesCoreAsync(bool notifyWhenUpToDate, bool showErrors)
    {
        ...
        try
        {
            ...
            UpdateStatusText = $"发现新版本 {result.LatestVersion}，当前版本 {result.CurrentVersion}。";
            var targetUrl = !string.IsNullOrWhiteSpace(result.DownloadUrl)
                ? result.DownloadUrl
                : result.ReleaseUrl;
            var confirmText = !string.IsNullOrWhiteSpace(result.DownloadUrl)
                ? "安装更新"
                : string.IsNullOrWhiteSpace(targetUrl) ? "知道了" : "打开发布页";
            var confirmed = await confirmationDialogService.ConfirmAsync(
                "发现新版本",
                BuildUpdateDialogMessage(result, targetUrl),
                confirmText,
                "稍后");
            ...
            if (confirmed && !string.IsNullOrWhiteSpace(result.DownloadUrl))
            {
                ...
                await notificationService.ShowInfoAsync(
                    "安装程序已启动",
                    $"已打开 {Path.GetFileName(installResult.InstallerPath)}。新版本安装并启动成功后，可以关闭当前程序。");
            }
            else if (confirmed && !string.IsNullOrWhiteSpace(result.ReleaseUrl))
            {
    ...
    private async Task StopGrabAsync()
    {
        try
        {
            ...
            await tomorrowReservationCoordinator.StopAsync();
        }
        catch (Exception ex)
        {
    ...
    [RelayCommand(CanExecute = nameof(CanRefreshReservationRecords))]
    ...
    private void RemoveCancelledReservationFromPresentation(ReservationInfo reservation)
    {
        var remaining = _reservationRecords
            .Where(record =>
                record.Kind != ReservationRecordKind.Today ||
                !string.Equals(record.ReservationToken, reservation.ReservationToken, StringComparison.Ordinal))
            .ToArray();
        UpdateReservationPresentation(remaining);
    }

    private void RemoveCancelledReservationFromPresentation(ReservationRecord reservation)
    {
        var remaining = _reservationRecords
            .Where(record => !IsSameReservationRecord(record, reservation))
            .ToArray();
        UpdateReservationPresentation(remaining);
    }

    private static bool IsSameReservationRecord(ReservationRecord candidate, ReservationRecord reservation)
    {
        if (candidate.Kind != reservation.Kind)
        {
            return false;
        }

        if (!string.IsNullOrWhiteSpace(candidate.ReservationToken) &&
            !string.IsNullOrWhiteSpace(reservation.ReservationToken))
        {
            return string.Equals(candidate.ReservationToken, reservation.ReservationToken, StringComparison.Ordinal);
        }

        return candidate.LibraryId == reservation.LibraryId &&
               string.Equals(candidate.SeatKey, reservation.SeatKey, StringComparison.Ordinal) &&
               candidate.ReservationDate == reservation.ReservationDate;
    }

    private static string GetReservationLogName(ReservationRecord reservation)
    {
        var kind = reservation.Kind == ReservationRecordKind.Today ? "今日预约" : "明日预约";
        var seatName = string.IsNullOrWhiteSpace(reservation.SeatName) ? reservation.SeatKey : reservation.SeatName;
        return string.IsNullOrWhiteSpace(reservation.LibraryName)
            ? $"{kind} {seatName}"
    ...
    private void OnReservationCountdownTick(object? sender, EventArgs e)
    {
        ...
        SyncSidebarCookieExpiryFromCurrentSession();
    ...
    {
        RefreshHomeReservationRecordViewModels(now);
    ...
    {
        var merged = new List<TrackedSeat>();
        var keys = new HashSet<string>(StringComparer.Ordinal);
        foreach (var seat in local.Concat(additions))
        {
            if (string.IsNullOrWhiteSpace(seat.SeatKey) || !keys.Add(seat.SeatKey))
            {
                continue;
            }
    ...
    {
        if (_reservationRecords.Count == 0)
        {
            return;
        }

        _homeReservationRecords.Clear();
        foreach (var record in _reservationRecords)
        {
            _homeReservationRecords.Add(new ReservationRecordViewModel(record, now, _appThemeService));
        }
    }
    ...
    private void SyncSidebarCookieExpiryFromCurrentSession()
    {
        var cookie = sessionService.CurrentSession?.Cookie;
        if (string.IsNullOrWhiteSpace(cookie))
        {
            return;
        }
        ...
        if (_sidebarCookieExpirationTime != expirationTime || !HasSidebarCookieExpiry)
        {
            _sidebarCookieExpirationTime = expirationTime;
            HasSidebarCookieExpiry = true;
        }
    ...
    private void UpdateReservationPresentation(IReadOnlyList<ReservationRecord> records)
    {
        var today = records.FirstOrDefault(record => record.Kind == ReservationRecordKind.Today && record.ExpirationTime is not null);
        _currentReservation = today?.ExpirationTime is not { } expirationTime
            ? null
            : new ReservationInfo(
        ...
        OnPropertyChanged(nameof(HasReservationRecords));
    ...
    private void UpdateReservationCountdown()
    {
        ...
        if (!HasActiveOccupyReservationCountdown)
        {
            ReservationCountdownText = "占座已停止";
            return;
        }
    ...
    private static string BuildUpdateDialogMessage(AppUpdateCheckResult result, string? targetUrl)
    {
        var builder = new StringBuilder()
            .AppendLine($"当前版本：{result.CurrentVersion}")
            .AppendLine($"最新版本：{result.LatestVersion}");

        if (!string.IsNullOrWhiteSpace(result.Notes))
        {
            builder.AppendLine()
                .AppendLine("更新说明：")
                .AppendLine(result.Notes.Trim());
        }

        if (string.IsNullOrWhiteSpace(targetUrl))
        {
            builder.AppendLine()
                .Append("更新清单没有提供下载地址，请稍后到 GitHub Releases 手动下载。");
        }
        else if (!string.IsNullOrWhiteSpace(result.DownloadUrl))
        {
            builder.AppendLine()
                .Append("确认后将下载安装程序，校验完成后自动打开安装向导。");
        }
        else
        {
            builder.AppendLine()
                .Append("确认后将打开 GitHub Releases 发布页。");
        }
