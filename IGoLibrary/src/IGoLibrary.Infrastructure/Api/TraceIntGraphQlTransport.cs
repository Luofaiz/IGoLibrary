...
public sealed class TraceIntGraphQlTransport(HttpClient httpClient, TraceIntRequestPolicy requestPolicy)
{
    ...
    public Task<HttpResponseMessage> SendAsync(string cookie, string payload, bool usePrereserveHeaders, CancellationToken cancellationToken = default)
    {
        return requestPolicy.ExecuteAsync(async requestToken =>
        {
            ...
            request.Headers.TryAddWithoutValidation("Referer", usePrereserveHeaders ? "https://web.traceint.com/" : "https://web.traceint.com/web/index.html");
            request.Headers.TryAddWithoutValidation("User-Agent", usePrereserveHeaders ? PrereserveQueueClient.TomorrowReservationUserAgent : DesktopUserAgent);
            request.Headers.TryAddWithoutValidation("App-Version", usePrereserveHeaders ? PrereserveQueueClient.TomorrowReservationAppVersion : "2.0.11");
