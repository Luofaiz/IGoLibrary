    ...
    TraceIntGraphQlTransport? graphQlTransport = null) : ITraceIntApiClient
{
    ...
    public async Task<IReadOnlyList<CommonSeat>> GetCommonSeatsAsync(string cookie, CancellationToken cancellationToken = default)
    {
        var templates = await protocolTemplateStore.GetEffectiveTemplatesAsync(cancellationToken);
        using var response = await SendGraphQlAsync(cookie, templates.QueryReservationInfoTemplate, cancellationToken);
        var raw = await response.Content.ReadAsStringAsync(cancellationToken);
        ...
        if (!document.RootElement.TryGetProperty("data", out var data) ||
            ...
            !userAuth.TryGetProperty("oftenseat", out var oftenSeat) ||
            !oftenSeat.TryGetProperty("list", out var list) ||
            list.ValueKind != JsonValueKind.Array)
        {
            return [];
        }

        var results = new List<CommonSeat>();
        foreach (var item in list.EnumerateArray())
        {
            var libraryId = ReadOptionalIntProperty(item, "lib_id");
            var seatKey = ReadOptionalStringProperty(item, "seat_key").Trim();
            if (libraryId <= 0 || string.IsNullOrWhiteSpace(seatKey))
            {
                continue;
            }

            var seatName = ReadOptionalStringProperty(item, "info").Trim();
            results.Add(new CommonSeat(libraryId, seatKey, string.IsNullOrWhiteSpace(seatName) ? seatKey : seatName));
        }

        return results.GroupBy(x => (x.LibraryId, x.SeatKey)).Select(x => x.First()).ToArray();
    }
