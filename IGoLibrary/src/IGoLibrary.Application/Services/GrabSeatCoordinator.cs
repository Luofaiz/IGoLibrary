    ...
    AppRuntimeState runtimeState) : IGrabSeatCoordinator
{
    ...
    private async Task RunAsync(GrabSeatPlan plan, CancellationToken cancellationToken)
    {
        try
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                var reservationResult = plan.UseRandomAvailableSeat
                    ? await TryReserveRandomAvailableSeatAsync(cookie, plan, random, MarkRequestSent, cancellationToken)
    ...
    private async Task<GrabReservationAttemptResult> TryReserveRandomAvailableSeatAsync(
        string cookie,
        GrabSeatPlan plan,
        Random random,
        Action markRequestSent,
        CancellationToken cancellationToken)
    {
        ...
        var availableSeats = layout.Seats
        ...
        var availableSeat = availableSeats[random.Next(availableSeats.Count)];
        activityLogService.Write(
            LogEntryKind.Success,
            "Grab",
            $"{availableSeat.SeatName} is available; selected randomly from {availableSeats.Count} available seats.");
