...
internal sealed class FakeTraceIntApiClient : ITraceIntApiClient
{
    ...
    public Task<LibraryLayout> GetPrereserveLibraryLayoutAsync(string cookie, int libraryId, CancellationToken cancellationToken = default)
        => OnGetPrereserveLibraryLayoutAsync?.Invoke(cookie, libraryId, cancellationToken)
           ?? throw new NotSupportedException();
    ...
    public Task<IReadOnlyList<CommonSeat>> GetCommonSeatsAsync(string cookie, CancellationToken cancellationToken = default)
        => OnGetCommonSeatsAsync?.Invoke(cookie, cancellationToken) ?? Task.FromResult<IReadOnlyList<CommonSeat>>([]);
    ...
    {
        if (OnGetReservationRecordsAsync is not null)
        {
            return await OnGetReservationRecordsAsync(cookie, cancellationToken);
        }

        if (OnGetReservationInfoAsync is null)
        {
            return [];
        }

        var info = await OnGetReservationInfoAsync(cookie, cancellationToken);
        return info is null
            ? []
            :
            [
                new ReservationRecord(
                    ...
                    info.ReservationToken,
                    info.LibraryId,
                    info.LibraryName,
                    info.SeatKey,
                    info.SeatName,
                    info.ExpirationTime,
                    DateOnly.FromDateTime(info.ExpirationTime.LocalDateTime.Date),
    ...
    public Task<PrereserveSaveResult> SavePrereserveSeatAsync(
        ...
        => OnSavePrereserveSeatAsync?.Invoke(cookie, libraryId, seatKey, cancellationToken)
           ?? Task.FromResult(new PrereserveSaveResult(false, cookie));
...
internal sealed class FakeConfirmationDialogService : IConfirmationDialogService
{
    ...
    public List<(string Title, string Message, string ConfirmText, string CancelText)> Requests { get; } = [];
    ...
    public Task<bool> ConfirmAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        Requests.Add((title, message, confirmText, cancelText));
...
{
    public string CurrentVersionText { get; set; } = "1.0.0";

    public AppUpdateCheckResult Result { get; set; } = new(false, "1.0.0", "1.0.0", null, null, null, null);
    ...
    public AppUpdateCheckResult? LastInstallUpdate { get; private set; }

    public AppUpdateInstallResult InstallResult { get; set; } = new(@"C:\Temp\IGoLibrarySetup.exe");
...
internal sealed class FakePrereserveQueueClient : IPrereserveQueueClient
{
    public List<string> Cookies { get; } = [];

    public Func<Func<PrereserveQueueMessage, CancellationToken, Task>, CancellationToken, Task>? OnRunAsync { get; set; }
    ...
    {
        Cookies.Add(cookie);
        return OnRunAsync?.Invoke(onMessageAsync, cancellationToken) ?? Task.Delay(Timeout.InfiniteTimeSpan, cancellationToken);
    }
}
...
internal sealed class FakeLibraryService : ILibraryService
{
    ...
    public IReadOnlyList<CommonSeat> CommonSeats { get; set; } = [];
...
internal sealed class FakeGrabSeatCoordinator : IGrabSeatCoordinator
{
    ...
    public GrabSeatPlan? StartedPlan { get; private set; }
...
internal sealed class FakeTomorrowReservationCoordinator : ITomorrowReservationCoordinator
{
    ...
    public TomorrowReservationPlan? StartedPlan { get; private set; }
...
internal sealed class FakeOccupySeatCoordinator : IOccupySeatCoordinator
{
    ...
    public OccupySeatPlan? StartedPlan { get; private set; }
