...
public sealed class GrabSeatCoordinatorTests
{
    ...
    [Fact]
    public async Task StartAsync_RandomAvailableSeatMode_ReservesOneAvailableSeatFromVenue()
    {
        var layoutCallCount = 0;
        var reservedSeatKeys = new List<string>();
        var reserved = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        var apiClient = new FakeTraceIntApiClient
        {
            OnGetLibraryLayoutAsync = (_, _, _) =>
            {
                ...
                return Task.FromResult(new LibraryLayout(
                    ...
                    1,
                    [
                        new SeatSnapshot("occupied", "1", true, 0, 0),
                        new SeatSnapshot("free-a", "2", false, 1, 0),
                        new SeatSnapshot("free-b", "3", false, 2, 0)
                    ]));
            },
            OnReserveSeatAsync = (_, _, seatKey, _) =>
            {
                reservedSeatKeys.Add(seatKey);
                reserved.TrySetResult();
                return Task.FromResult(true);
            }
        };
        ...
        var reservedSeatKey = Assert.Single(reservedSeatKeys);
        Assert.Contains(reservedSeatKey, new[] { "free-a", "free-b" });
        Assert.DoesNotContain("occupied", reservedSeatKeys);
        Assert.Equal(1, layoutCallCount);
        Assert.NotNull(runtimeState.CurrentLayout);
    }
