...
public sealed class OccupySeatCoordinatorTests
{
    ...
    [Fact]
    public async Task StartAsync_CompletesWithoutCancelling_WhenReservationIsCheckedIn()
    {
        var cancelCalls = 0;
        var reserveCalls = 0;
        var reservation = new ReservationInfo(
        ...
        await coordinator.StartAsync(new OccupySeatPlan(TimeSpan.FromSeconds(60), RefreshMode.FixedTenSeconds));
        await WaitForStatusAsync(coordinator, CoordinatorTaskState.Completed);

        Assert.Equal(0, cancelCalls);
        Assert.Equal(0, reserveCalls);
        Assert.Same(reservation, runtimeState.CurrentReservation);
        Assert.Contains("学习中", coordinator.GetStatus().Message);
        Assert.Contains(activityLogService.Entries, entry => entry.Category == "Occupy" && entry.Message.Contains("学习中"));
    }
